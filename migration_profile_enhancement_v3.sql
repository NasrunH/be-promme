...
-- Versi 3 - Fixed semua casting issues
...
ADD COLUMN IF NOT EXISTS bahasa text[] DEFAULT ARRAY['Indonesia']::text[],
...
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Error updating jenis_kelamin: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Error updating ukuran_perusahaan: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Error updating industri: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Error updating kategori_niche: %', SQLERRM;
END $$;
...
BEGIN
    ...
    -- Set default value baru dengan casting yang benar
    ALTER TABLE public.creators ALTER COLUMN bahasa SET DEFAULT ARRAY['Indonesia']::bahasa_enum[];
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Error updating bahasa: %', SQLERRM;
END $$;
